	...
	struct mmc_blk_request *brq, int *ecc_err, int *gen_err)
{
	...
	if (R1_CURRENT_STATE(status) == R1_STATE_DATA ||
		...
		if (err) {
			 ...
			 */
			mmc_card_set_removed(card); /* remove card */
			card->host->unused = 1; /* avoid re-detect by .get_cd */
