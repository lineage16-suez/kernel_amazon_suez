...
struct mmc_host *sd_mmc_host = NULL;
...
void sd_thermal_detect_change(void)
{
	if (sd_mmc_host == NULL || sd_mmc_host->card == NULL)
		return;
	mmc_card_set_removed(sd_mmc_host->card);
	/* set sdcard to unused due to thermal event, reset after card eject */
	sd_mmc_host->unused = 1;
	_mmc_detect_change(sd_mmc_host, 0, 0);
}
EXPORT_SYMBOL(sd_thermal_detect_change);
