...
//	char *hwconnected[2]    = { "USB_STATE=HWCONNECTED", NULL };
	...
	pr_debug("[USB]%s: is_hwconnected=%d \n", __func__, is_hwconnected);
	/* Add for HW/SW connect */
	...
	if (uevent_envp) {
		...
		pr_debug("[USB]%s: did not send uevent (%d %d %p)\n", __func__,
	...
	if (uevent_envp_cdrom) {
		...
		pr_debug("[USB]%s: did not send zero uevent\n", __func__);
	}
			       ...
			       struct usb_configuration *c)
{
	...
	list_for_each_entry(f, &dev->enabled_functions, enabled_list) {
		pr_debug("[USB]unbind_config function '%s'/%p\n", f->name, f);
		if (f->unbind_config)
			f->unbind_config(f, c);
	}
}
...
static int android_enable_function(struct android_dev *dev, char *name)
{
	...
	while ((f = *functions++)) {
		...
		/* Added for USB Develpment debug, more log for more debuging help */
		pr_debug("[USB]%s: name = %s, f->name=%s \n", __func__, name, f->name);
		/* Added for USB Develpment debug, more log for more debuging help */
			    ...
			    const char *buff, size_t size)
{
	...
	if (get_boot_mode() == KERNEL_POWER_OFF_CHARGING_BOOT || get_boot_mode() == LOW_POWER_OFF_CHARGING_BOOT) {
		...
		list_for_each_entry(f, &dev->enabled_functions, enabled_list) {
			pr_debug("[USB]enable function '%s'/%p\n", f->name, f);
			if (f->enable)
				f->enable(f);
		}
		...
		if (!usb_cable_connected()) {
			...
			pr_debug("[USB]%s: enable 0->0 case - no usb cable", __func__);
		}
		/* Add for HW/SW connect */
	}
