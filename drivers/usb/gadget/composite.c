...
composite_setup(struct usb_gadget *gadget, const struct usb_ctrlrequest *ctrl)
{
	...
	case USB_REQ_GET_DESCRIPTOR:
		...
		case USB_DT_DEVICE:
			...
			DBG(cdev, "[COM]USB_REQ_GET_DESCRIPTOR: "
		...
		case USB_DT_BOS:
			...
			break;
		default:
			DBG(cdev, "[COM]USB_REQ_GET_DESCRIPTOR w_value=0x%X\n", w_value);
			break;
		}
		break;
	...
	case USB_REQ_SET_CONFIGURATION:
		...
		DBG(cdev, "[COM]USB_REQ_SET_CONFIGURATION: "
						"value=%d\n",value);
		break;
	case USB_REQ_GET_CONFIGURATION:
		...
		DBG(cdev, "[COM]USB_REQ_GET_CONFIGURATION: "
						"value=%d\n", value);
	...
	case USB_REQ_SET_INTERFACE:
		...
		DBG(cdev, "[COM]USB_REQ_SET_INTERFACE: "
						"value=%d\n", value);
		spin_unlock(&cdev->lock);
		break;
	case USB_REQ_GET_INTERFACE:
		...
		*((u8 *)req->buf) = value;
		...
		DBG(cdev, "[COM]USB_REQ_GET_INTERFACE: "
						"value=%d\n", value);
		break;
	...
	case USB_REQ_GET_STATUS:
		if (__ratelimit(&ratelimit))
			DBG(cdev, "[COM]USB_REQ_GET_STATUS\n");
		if (!gadget_is_superspeed(gadget))
			goto unknown;
		if (ctrl->bRequestType != (USB_DIR_IN | USB_RECIP_INTERFACE))
			goto unknown;
	...
	case USB_REQ_SET_FEATURE:
		if (__ratelimit(&ratelimit))
			DBG(cdev, "[COM][ratelimit]%s w_value=%d\n",
					((ctrl->bRequest == USB_REQ_SET_FEATURE) ?
		...
		case USB_INTRF_FUNC_SUSPEND:
			DBG(cdev, "[COM]USB_INTRF_FUNC_SUSPEND\n");
			if (!cdev->config || intf >= MAX_CONFIG_INTERFACES)
				break;
			f = cdev->config->interface[intf];
			if (!f)
...
unknown:
	...
	if(value < 0) {
		DBG(cdev, "[COM]composite_setup: value=%d,"
